  ...
  const VALUE_TESTS: Array<{
    ...
    },
    {
      ...
      tests: [
        ...
        {
          title: "should coerce a comma-and-space-separated values into a string list",
          env: "first, second, third",
          expect: ["first", "second", "third"],
        },
        {
    ...
    describe(`${group.method.name}().value`, () => {
      for (const test of group.tests) {
        it(test.title, () => {
          ...
          if (test.throws) {
            expect(() => group.method(TEST_PARAM, test.options).value).to.throw();
          } else {
            expect(group.method(TEST_PARAM, test.options).value).to.deep.eq(test.expect);
          }
        });
      }
    });
  ...
  describe("Param", () => {
    describe("#toSpec()", () => {
      it("should cast non-string defaults to strings", () => {
        expect(new Param(TEST_PARAM, { default: 123 }).toSpec().default).to.eq("123");
      });

      it("should passthrough supplied options", () => {
        expect(new Param(TEST_PARAM, { description: "hello expect" }).toSpec().description).to.eq(
          "hello expect"
        );
      });
