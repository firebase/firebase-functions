...
describe("Cloud Firestore (v2)", () => {
  ...
  describe("Document updated trigger", () => {
    ...
    it("should have the correct data", async () => {
      // Retry getting the data snapshot to ensure the function has processed
      const finalSnapshot = await retry(() => docRef.get());
      expect(finalSnapshot.data()).toStrictEqual({ test: testId });
    });
  });
